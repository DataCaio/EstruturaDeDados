...
void Escalonador::executarSimulacao(Pacote* pacotes, int numPacotes, Armazem* armazens, int numArmazens, Transporte& transporte, int latencia, int intervalo, int capacidade) {
    ...
    while (!this->eventos.vazio()) {
        ...
        if (proximo_evento.getTipo() == 1) { // Chegada de Pacote
        ...
        } else if (proximo_evento.getTipo() == 2) { // Transporte
